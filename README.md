...
> **Status (2026-09-24):** the pipeline is built and tested end-to-end on synthetic data, but the live search
> has **not** run yet. The build environment's network policy blocks every portal, Apify, OpenStreetMap and
> the central bank API, and no Apify token is configured. See [What is needed to run the live search](#what-is-needed-to-run-the-live-search).

## Deliverables

| File | What it is |
|---|---|
| `outputs/Miraflores_Rental_Shortlist.xlsx` | 7 sheets: EXECUTIVE_SHORTLIST, ALL_MATCHES, STRETCH_NEGOTIABLE, NEAR_MISSES, SOURCE_AUDIT, METHODOLOGY, CONTACT_GUIDE — clickable *View listing / Open WhatsApp / Open map* links, filters, frozen panes, conditional formatting (Top 5, noise risk, over-budget, UNKNOWN) |
| `outputs/Miraflores_Rental_Executive_Report.pdf` (+ `.html`) | 5–10 page English report: requirements, sources, market snapshot, Top 10 cards, quiet-space and value picks, map, lease observations, verification checklist, methodology |
| `outputs/Contact_Templates.txt` | English + Spanish messages (full and WhatsApp-length). Nothing is ever sent automatically |
...
The live `outputs/` files are created by `--mode full`. The pipeline refuses to write a client deliverable from
zero listings.

## What is needed to run the live search

1. **Network access.** The cloud environment must be allowed to reach:
   `api.apify.com`, `urbania.pe`, `www.adondevivir.com`, `inmuebles.mercadolibre.com.pe`,
   `departamento.mercadolibre.com.pe`, `overpass-api.de`, `nominatim.openstreetmap.org`, `estadisticas.bcrp.gob.pe`.
   In Claude Code on the web: environment menu → *Edit* → *Network access* (or run locally, where no policy applies).
2. **Apify token.** One line, never committed:
   ```bash
   echo "APIFY_TOKEN=apify_api_XXXXXXXX" > .env
   ```
3. Run:
   ```bash
   pip install -r requirements.txt
   python -m src.pipeline --mode validate   # Gates 0–3: 15 records/source, field-coverage audit (≈ cents)
   python -m src.pipeline --mode full       # Gates 0–10: full search → outputs/
   ```

Without a token, Urbania/Adondevivir get one polite direct request each, which stops at the first bot challenge.
Mercado Libre and OpenStreetMap need no token.
